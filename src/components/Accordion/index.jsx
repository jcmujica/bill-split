      ...
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className={`text-left text-xl font-bold font-[RobotoSlab] ${isOpen ? 'text-green-500' : 'text-gray-500'}`}>
              {title}
            </span>
          </div>
          <span className={`text-green-500 ${isOpen ? 'text-green-500' : 'text-gray-500'}`}>{isOpen ? '-' : '+'}</span>
        </div>
      </div>
      {isOpen && <div className="p-4 border bg-green-100 dark:bg-slate-950 border-slate-300 dark:border-slate-800 rounded-xl">
        {children}
      </div>}
    </div>
  );
...
const StepNumber = ({ step, currentStep }) => {
  ...
  return (
    <div className="flex items-center justify-center text-xs font-bold text-slate-100 dark:text-slate-800">
      <span className={`rounded-full text-bold text-lg flex items-center justify-center w-6 h-6 ${isActive ? 'bg-green-500' : 'bg-gray-500'}`}>
        {step}
      </span>
    </div>
  );
